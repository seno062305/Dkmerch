...
const ProductModal = ({ product, onClose, onAddToCart, onAddToWishlist }) => {
  ...
  const handleSubmitReview = () => {
    ...
    if (review.length > 250) {
      showNotification('Review must be 250 characters or less', 'error');
      return;
    }
    ...
    const existingIndex = reviews.findIndex(
      r => r.productId === product.id && r.userEmail === user.email
    );
  ...
  const getAverageRating = () => {
    ...
    const sum = existingReviews.reduce((acc, r) => acc + r.rating, 0);
    return (sum / existingReviews.length).toFixed(1);
  };
            ...
            <div className="modal-details-section">
              <div className="modal-product-info">
                  ...
                  <div className="average-rating">
                    <div className="stars">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <i
                          key={star}
                          className={`fas fa-star ${star <= Math.round(getAverageRating()) ? 'filled' : ''}`}
                        ></i>
                      ))}
                    </div>
                ...
                <div className="modal-actions">
                  {product.isPreOrder ? (
                    <>
                      <button 
                        className="btn btn-preorder" 
                        onClick={handlePreOrderClick}
                        disabled={product.stock === 0}
                      >
                        <i className="fas fa-shopping-cart"></i> Pre-Order Now
                      </button>
                      <button className="btn btn-wishlist-outline" onClick={handleWishlistClick}>
              ...
              <div className="review-section">
                <h3>
                  ...
                  {hasUserReviewed ? 'Your Review' : 'Rate & Review'}
                </h3>
                    ...
                    <div className="rating-input">
                      ...
                      <div className="stars-input">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <i
                            key={star}
                            className={`fas fa-star ${star <= (hoverRating || rating) ? 'active' : ''}`}
                        ...
                        <div className="review-header">
                          ...
                          <div className="review-stars">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <i
                                key={star}
                                className={`fas fa-star ${star <= rev.rating ? 'filled' : ''}`}
                              ></i>
                            ))}
                          </div>
                        </div>
